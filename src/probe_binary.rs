...
#[display(fmt="Data was unexpected length for nz = {}. Length was {} but expected a length of {} ({} difference)",
    nz, length, expected_length, difference
)]
...
pub(crate) fn parse_file_group<W: Write>(files: &[&Path], nz: usize, writer: W) -> Result<(), ProbeBinaryError> {
    let parse_results :Result<Vec<(usize, ProbeFile)>, _> = 
    files
        .into_par_iter()
        .enumerate()
        .map(|(idx, path)| {
            // load and parse the binary file in parallel
            parse_binary_file(path, nz).map(|x| (idx, x))
        })
        .collect();
    ...
    for (idx, slice) in parse_results? {
        viscous.slice_mut(ndarray::s![idx, .., ..]).assign(&slice.viscous);
        log_law.slice_mut(ndarray::s![idx, .., ..]).assign(&slice.log_law);
        freestream.slice_mut(ndarray::s![idx, .., ..]).assign(&slice.freestream);
    }

    // assemble the data and write it out to a file
    let probes = ProbeFileAllTimesteps { viscous, log_law, freestream };
...
fn parse_binary_file(path: &Path, nz: usize) -> Result<ProbeFile, ProbeBinaryError> {
    ...
    Ok(ProbeFile { viscous, log_law, freestream })
}
