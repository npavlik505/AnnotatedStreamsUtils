...
pub(crate) fn hdf5_to_vtk(args: cli::HDF5ToVtk) -> Result<()> {
    ...
    if vtk_output_folder.exists() {
        std::fs::remove_dir_all(&vtk_output_folder)
            .with_context(|| format!("failed to clear vtk output folder at {}", vtk_output_folder.display()))?;
    }

    std::fs::create_dir(&vtk_output_folder)
            .with_context(|| format!("failed to create vtk output folder at {}", vtk_output_folder.display()))?;
    ...
    let dset = file.dataset("velocity")
        .with_context(|| format!("dataset `velocity` was missing from h5 file {}", flowfields_file.display()))?;
    ...
    let vtk_mesh : vtk::Mesh3D<_, vtk::Binary> = vtk::Mesh3D::new(mesh.x_data, mesh.y_data, mesh.z_data);
    ...
    let _ : Result<()> = (0..nwrite).into_par_iter()
        .map(|write| -> Result<(), anyhow::Error> {
            ...
            for i in 0..nx {
                for j in 0..ny {
                    for k in 0..nz {
                        ...
                        vtk_container.data.energy[[i,j,k]] = rho_energy_ref[[i,j,k]] / rho_ref[[i,j,k]];

                        for v in 0..3 {
                            vtk_container.data.velocity[[v, i,j,k]] = rho_velocity_ref[[v, i,j,k]] / rho_ref[[i,j,k]];
                        }
                    }
                }
            }
            ...
            let file = std::fs::File::create(&write_path)
                .with_context(|| format!("unable to create vtr output at {}", write_path.display()))?;
